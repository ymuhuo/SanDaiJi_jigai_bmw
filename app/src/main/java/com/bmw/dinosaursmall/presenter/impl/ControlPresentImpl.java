...
public class ControlPresentImpl implements ControlPresenter {
    ...
    private long time_get_gas;
    private long time_get_temp;
...
//        socketUtil = new SocketUtil(preview, context);
        ...
        initSharedPreferences(true, true);
...
//        fixedThreadPool.execute(environmentRunnable);
        new Thread(batteryRunnable).start();
        new Thread(posturerRunnable).start();
        new Thread(environmentRunnable).start();

       /* new Thread(new Runnable() {
            @Override
            public void run() {
                sleep(3000);
                log("adsflsdjfkl",false);
    ...
    private void initSocket() {
        ...
        String ip = LoginInfo.getInstance().getSocket_ip();
        int port = LoginInfo.getInstance().getSocket_port();

        socketUtil.socketLogin(ip, port, 20108);
        listenSocketResult();
    }
        ...
        environmentRunnable = new Runnable() {
            ...
            public void run() {
                while (isGetDeviceInfo) {
                    ...
                    getGasInfo();
                    sleep(140);
                    if (System.currentTimeMillis() - time_get_gas >= 1000 * 15) {
                        initSharedPreferences(true, false);
                    }

                    getHumidityInfo();
                    sleep(140);
                    if (System.currentTimeMillis() - time_get_temp >= 1000 * 15) {
                        initSharedPreferences(false, true);
                    }
                    sleep(1500 * 1);
                }
            }
    ...
    private void initSharedPreferences(boolean resetGas, boolean resetTemp) {
//        SharedPreferences.Editor editor = sharedPreferences.edit();
        if (resetGas) {
            environmentInfo.setmCH4(-10000);
            environmentInfo.setmCO(-10000);
            environmentInfo.setmCO2(-10000);
            environmentInfo.setmO2(-10000);
...
//            editor.putFloat(Environment.O2, -10000);
        ...
        if (resetTemp) {
            environmentInfo.setmHUMIDITY(-10000);
            environmentInfo.setmTEMPERATURE(-10000);
...
//        editor.clear();
        ...
        preview.setEnvironmentData(environmentInfo);
    }
...
//        commandOprater(0x09, 0x04, new int[]{left_low, left_high, right_low, right_high}, true);
    ...
    @Override
    public void openBigLight() {
        sendBigLightCommands(1);
    }

    @Override
    public void closeBigLight() {
        sendBigLightCommands(0);
    }

    private void sendBigLightCommands(int lightNum) {
        int sum = (control_addr + 0x8e + 0x01 + lightNum) % 0x100;
        commands = new byte[]{(byte) 0x8e, (byte) lightNum};
        byte[] result = new byte[6];
        getReader(result);
    }

    @Override
    public void openFireExtingguisher() {
        sendFireCommands(1);
    }

    @Override
    public void closeFireExtingguisher() {
        sendFireCommands(0);
    }

    private void sendFireCommands(int fireNum) {
        int sum = (control_addr + 0x9e + 0x01 + fireNum) % 0x100;
        commands = new byte[]{(byte) 0x9e, (byte) fireNum};
        byte[] result = new byte[6];
        getReader(result);
    }
    ...
    private void getRoofModule_info(int i, int byte_num) {
        int sum = (control_addr + 1 + 0x14 + i) % 0x100;
        commands = new byte[]{(byte) 0x7e, (byte) control_addr, (byte) 0x14, 0x01, (byte) i,
                (byte) sum};
    ...
    private void sendCommand(int byte_nu) {
        ...
        if (isSendCommand) {
            socketUtil.sendCommandInThread(commands, 1);
        }
    ...
    private void sendCommand2(int byte_nu) {
        if (isSendCommand) {
            socketUtil.sendCommandInThread(commands, 2);
        }
            /*socketUtil.getReader(commands, new byte[byte_nu], new SocketReaderListener() {
                @Override
                public void Result(byte[] bytes) {
    ...
    private void getReader(byte[] result) {
        if (isGetCommand) {
//            log("send command sldfsld;fjsla;",false);
            socketUtil.sendCommandInThread(commands, 5);
        }
    ...
    private void handler_bytesProcessor(byte[] bytes) {
        switch (bytes[2] & 0xff) {
            ...
            case 0x91:  //获取电量信息
                ...
                break;
            case 0x92:  //获取摆臀角度
                if (bytes[6] != 0 && bytes[3] == 0x02) {
                    swing_angle = bytes[5] << 8 | (bytes[4] & 0xff);
                }
                break;
            case 0x93:  //获取姿态数据
                if (bytes[10] != 0 && bytes[3] == 0x06) {
...
//        ra_picth = new RotateAnimation(50, 50, Animation.RELATIVE_TO_SELF, 0.63f, Animation.RELATIVE_TO_SELF, 0.5f);
//        ra_picth = new RotateAnimation(-30, -30, Animation.RELATIVE_TO_SELF, 0.37f, Animation.RELATIVE_TO_SELF, 0.5f);
        /**
         * 手机：0.84
         * 平板：0.84
         * */
...
//        ra_swing = new RotateAnimation(-60, -60, Animation.RELATIVE_TO_SELF, 0.84f, Animation.RELATIVE_TO_SELF, 0.53f);
...
//        ra_swing.setFillAfter(true);
    ...
    private void dealEnvironment_info(byte[] bytes, int which) {
        SharedPreferences.Editor editor = sharedPreferences.edit();
        if (which == 0) {
            time_get_gas = System.currentTimeMillis();
            float mCO_num = (bytes[4] & 0xff) | (bytes[5] & 0xff << 4);
            float mCH4_num = (bytes[6] & 0xff) | (bytes[7] & 0xff << 4);
            float mO2_num = (bytes[8] & 0xff) | (bytes[9] & 0xff << 4);
            float mCO2_num = (bytes[10] & 0xff) | (bytes[11] & 0xff << 4);

            if (mCH4_num != 0) {
                if (mCH4_num <= 404) {
                    mCH4_num = 0;
                } else {
                    mCH4_num = (mCH4_num - 404) * 50 / 803;
                }
                if (mCH4_num >= 100) {
                    mCH4_num = 100;
                }
            ...
            if (mCO2_num != 0) {
                if (mCO2_num <= 466) {
                    mCO2_num = 400 - (466 - mCO2_num) * 727 / 4600;
                } else {
                    mCO2_num = 400 + (mCO2_num - 466) * 4600 / 727;
                }
            } else {
                mCO2_num = -10000;
            }
            ...
            if (mO2_num != 0) {
                if (mO2_num >= 1080) {
                    mO2_num = (float) (10 - (mO2_num - 1080) * 10.9 / 520);
                } else {
                    mO2_num = (float) (10 + (1080 - mO2_num) * 10.9 / 520);
                }
            } else {
                mO2_num = -10000;
            }
...
//            editor.putFloat(Environment.CO2, mCO2_num);
            environmentInfo.setmCO(getDecimalDit(mCO_num,100));
            environmentInfo.setmCH4(getDecimalDit(mCH4_num,100));
            environmentInfo.setmO2(getDecimalDit(mO2_num,100));
            environmentInfo.setmCO2(getDecimalDit(mCO2_num,100));

            preview.ilog("成功装载所有气体环境数据！" + mCO_num + "  " + mCH4_num + " " + mO2_num + " " + mCO2_num);
        } else {
            time_get_temp = System.currentTimeMillis();
            float mTemparature_num = (bytes[4] & 0xff) | (bytes[5] & 0xff << 4);
            float mHumidity_num = (bytes[6] & 0xff) | (bytes[7] & 0xff << 4);

//            editor.putFloat(Environment.TEMPERATURE, mTemparature_num);
//            editor.putFloat(Environment.HUMIDITY, mHumidity_num);
            environmentInfo.setmTEMPERATURE(mTemparature_num);
            environmentInfo.setmHUMIDITY(mHumidity_num);

            preview.ilog("成功装载所有温湿度环境数据！" + mTemparature_num + "  " + mHumidity_num);
        }
//        editor.commit();
//        editor.clear();
        preview.setEnvironmentData(environmentInfo);
